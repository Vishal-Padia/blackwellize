...
def gemm_multi_cta(
    ...
    if warp_idx == 0:
        ...
        # GMEM index 0 is the only K-tile, and becomes the loop variable on the
        # next rung. The SMEM side takes no index: this layout carries no stage
        # mode, so grouping left it rank 1.
        cute.copy(tma_atom_a, tAgA[(None, 0)], tAsA[None], tma_bar_ptr=ab_full_mbar)
        cute.copy(tma_atom_b, tBgB[(None, 0)], tBsB[None], tma_bar_ptr=ab_full_mbar)
...
@cute.jit
def gemm_multi_cta_host(a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    m, _ = a.shape
    n, _ = b.shape
...
def run(m: int = 256, n: int = 512, k: int = 16):
    """One UMMA per CTA over an m x n grid of tiles, checked against torch."""
    import torch
    from cutlass.cute.runtime import from_dlpack

    tile_m, tile_n, tile_k = mma_tiler_mnk
    # K is still exactly one instruction (no K-loop until the next rung), but M and
    # N are free as long as they tile evenly -> grid = (m/tile_m, n/tile_n)
    if k != tile_k or m % tile_m or n % tile_n:
        raise ValueError(
            f"need k=={tile_k}, m%{tile_m}==0, n%{tile_n}==0; got {(m, n, k)}"
        )
    ...
    compiled = cute.compile(gemm_multi_cta_host, a_t, b_t, c_t)
