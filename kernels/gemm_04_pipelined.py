...
def gemm_pipelined(
    ...
    if warp_idx == 0:
        ...
        for s in cutlass.range_constexpr(num_stages):
            with cute.arch.elect_one():
                cute.arch.mbarrier_arrive_and_expect_tx(ab_full_mbar + s, nbytes)
            cute.copy(
                tma_atom_a,
                tAgA[(None, s)],
                tAsA[(None, s)],
                tma_bar_ptr=ab_full_mbar + s,
            )
            cute.copy(
                tma_atom_b,
                tBgB[(None, s)],
...
@cute.jit
def gemm_pipelined_host(a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    m, _ = a.shape
    n, _ = b.shape
...
def run(m: int = 256, n: int = 512, k: int = 4096, iters: int = 50):
    ...
    compiled = cute.compile(gemm_pipelined_host, a_t, b_t, c_t)
    ...
    for name, secs in (("gemm_04", ours), ("torch", theirs)):
