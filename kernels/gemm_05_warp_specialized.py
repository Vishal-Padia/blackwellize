...
def gemm_warp_specialized(
    ...
    a_smem_ptr = cute.arch.alloc_smem(
        io_dtype, num_stages * mma_tiler_mnk[0] * mma_tiler_mnk[2], 128
    )
    b_smem_ptr = cute.arch.alloc_smem(
        io_dtype, num_stages * mma_tiler_mnk[1] * mma_tiler_mnk[2], 128
    )

    # full[s]  -> slot s has been loaded, safe to read
    # empty[s] -> the MMA that was reading slot s has retired, safe to overwrite
    ...
    # used exactly once, after the loop, to drain the MMAs still in flight
    ...
    sA = cute.make_tensor(
        ...
        cute.append(
            a_smem_layout,
            cute.make_layout(num_stages, stride=(cute.cosize(a_smem_layout))),
        ),
    )
    sB = cute.make_tensor(
        b_smem_ptr,
        cute.append(
            b_smem_layout,
            cute.make_layout(num_stages, stride=(cute.cosize(b_smem_layout))),
        ),
    )
    ...
    if warp_idx == 0:
        ...
        for k_tile in cutlass.range(num_k_tiles, unroll=1):
            ...
            cute.gemm(
                tiled_mma,
                tCtAcc,
                tCrA[None, None, 0, stage],
                tCrB[None, None, 0, stage],
                tCtAcc,
            )
            with cute.arch.elect_one():
...
@cute.jit
def gemm_warp_specialized_host(a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    m, _ = a.shape
    n, _ = b.shape
...
def run(m: int = 256, n: int = 512, k: int = 4096, iters: int = 50):
    ...
    compiled = cute.compile(gemm_warp_specialized_host, a_t, b_t, c_t)
    ...
    print("gemm_05_warp_specialized: ok")
    ...
    for name, secs in (("gemm_05", ours), ("torch", theirs)):
