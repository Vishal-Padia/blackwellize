...
tmem_cols = 512
num_stages = 4
# the 64-deep tile is four 16-deep MMA instructions
...
def gemm_swizzling(
    ...
    # 1024-byte aligned: the 128B swizzle pattern spans 8 rows x 128B = 1024B
...
@cute.jit
def gemm_swizzling_host(a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    m, _ = a.shape
    n, _ = b.shape
...
def run(m: int = 256, n: int = 512, k: int = 4096, iters: int = 50):
    ...
    compiled = cute.compile(gemm_swizzling_host, a_t, b_t, c_t)
    ...
    for name, secs in (("gemm_06", ours), ("torch", theirs)):
