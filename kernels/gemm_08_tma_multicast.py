...
# CTAs that write into our buffers: ourselves, the N-peer (our A tile) and the
# M-peer (our B tile). Every one of them must release the slot before a producer
# may overwrite it, so the empty barrier collects that many arrivals.
num_empty_arrivals = cluster_shape_mn[0] + cluster_shape_mn[1] - 1


@cute.kernel
def gemm_tma_multicast(
    ...
    cta_rank_m, cta_rank_n, _ = cute.arch.block_in_cluster_idx()
    ...
    a_mcast_mask = cpasync.create_tma_multicast_mask(
        ...
        mcast_mode=2,  # vary N -> all ctas that share our A tile
    )

    b_mcast_mask = cpasync.create_tma_multicast_mask(
        cluster_layout_vmnk,
        block_in_cluster_coord_vmnk,
        mcast_mode=1,  # vary M -> all ctas that share our B tile
    )

    # everyone who writes into our slots, so the consumer can release all of them
    empty_mcast_mask = a_mcast_mask | b_mcast_mask
    ...
    if warp_idx == 0:
        with cute.arch.elect_one():
            for s in cutlass.range_constexpr(num_stages):
                ...
                # pre-fill so the producer's first lap does not block
    ...
    tAsA, tAgA = cpasync.tma_partition(
        tma_atom_a,
        cta_rank_n,  # rank within A's multicast group
        cute.make_layout(2),
    ...
    tBsB, tBgB = cpasync.tma_partition(
        tma_atom_b,
        cta_rank_m,  # rank within B's multicast group
        cute.make_layout(2),
    ...
    if warp_idx == 0:
        ...
        for k_tile in cutlass.range(num_k_tiles, unroll=1):
            ...
            with cute.arch.elect_one():
                tcgen05.commit(ab_empty_mbar + stage, empty_mcast_mask)
...
@cute.jit
def gemm_tma_multicast_host(a: cute.Tensor, b: cute.Tensor, c: cute.Tensor):
    m, _ = a.shape
    n, _ = b.shape
    ...
    cluster_layout_vmnk = cute.tiled_divide(
        ...
        (tiled_mma.thr_id.shape,),  # 1 for CTAGroup.One
    )
    gemm_tma_multicast(
        tiled_mma,
        tma_atom_a,
...
def run(m: int = 256, n: int = 512, k: int = 4096, iters: int = 50):
    ...
    compiled = cute.compile(gemm_tma_multicast_host, a_t, b_t, c_t)
    ...
    print("gemm_08_tma_multicast: ok")
    ...
    for name, secs in (("gemm_08", ours), ("torch", theirs)):
